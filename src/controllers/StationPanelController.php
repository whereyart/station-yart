...
class StationPanelController extends \BaseController {
	...
	public function do_delete($panel_name, $ids){
		...
		$panel			= new Panel;
		$user_scope 	= $panel->user_scope($panel_name, 'D', $this->subpanel_parent);

		$override_response = $this->override_responded_to($user_scope, 'D',$ids);
		if ($override_response) return $override_response;
	...
	private function override_responded_to($user_scope, $method,$id=0){
		...
		if ($override)
		{
			...
			if($id) $override['method'] = str_replace('%id%', $id, $override['method']);
